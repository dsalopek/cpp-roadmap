...
namespace
{
    ...
    uint8_t toUint8(Sensors::Status status)
    {
        return static_cast<uint8_t>(status);
    }

    Sensors::Status toStatus(uint8_t value)
    {
        return static_cast<Sensors::Status>(value);
    }
...
namespace Sensors
{
    ...
    Sensors::Status toggleStatus(Sensors::Sensor &sensor, Sensors::Status toToggle)
    {
    ...
    {
        if (sensor.sensorStatus() & toUint8(status))
        {
            return positive;
        }
        return negative;
    ...
    {
        return out << sensor.sensorMetadata().sensorDisplayName << " (" << sensor.sensorName() << ") | value: " << sensor.sensorValue() << sensor.sensorMetadata().sensorUnits << " | status flags: " << sensor.sensorStatusDisplay();
    }
    ...
    Sensor::Sensor(const std::string &name,
                   const SensorMetadata &sensorMetadata,
                   const double initialValue, const std::uint8_t sensorStatus) : m_sensorName{name},
                                                                                 m_sensorValue{initialValue},
                                                                                 m_sensorStatus{
                                                                                     sensorStatus},
                                                                                 m_sensorMetadata{
